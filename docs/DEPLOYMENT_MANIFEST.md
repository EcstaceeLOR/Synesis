...
`@synesis/olas-contracts` is the source of truth for contracts and calldata that may move value. The active `base-2026-09-15.2` manifest (`sha256:7601b8c8bce4817d5a7a376ee354f8da55f3ec633b833713d973fd5966d91c7a`) is canonical-JSON encoded, content-hashed, and recursively frozen. An intent stores both `manifestVersion` and `contentHash`; `assertManifestReference` rejects a version or content mismatch. The previous `.1` manifest remains exported with its original `sha256:e585859ea3792a6838e99aa6eb4047f017001bac4b983b12717d791b1fc6f927` hash.
...
| Olas USDC balance tracker | `0x0443c55e151dba13fae079518f9dd01ff9c21cb2` | direct deployment                            | runtime code        |
...
The manifest also pins SHA-256 hashes of runtime bytecode at all seven addresses. Integration onboarding calls `verifyDeploymentHealth` and remains not ready if the RPC is not Base mainnet or any direct code, proxy code, implementation pointer, or implementation code differs. This makes an upstream upgrade visible and fail-closed.
...
| USDC approval | `approve(address,uint256)`                             | `0x095ea7b3` | Olas tracker ≤1 USDC or Aave Pool ≤10,000 USDC                    |
